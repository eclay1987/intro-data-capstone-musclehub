...
ab_test = lambda x: 'A' if x.fitness_test_date != None else 'B'
...
ab_counts = ab_counts.rename(columns={'visit_date': 'counts'})
print(ab_counts)
...
plt.pie(ab_counts.counts,autopct='%0.2f%%')
plt.axis('equal')
...
app_test = lambda x: 'Application' if x.application_date != None else 'No Application'
df['is_application'] = df.apply(app_test, axis=1)
print(df.head(10))
...
purch_test = lambda x: 'Member' if x.purchase_date != None else 'Not Member'
df['is_member'] = df.apply(purch_test, axis=1)
print(df.head(10))
...
final_member_counts = df.groupby(['ab_test_group', 'is_member']).visit_date.count().reset_index()
print(final_member_counts)
final_member_pivot = final_member_counts.pivot(columns='is_member', index='ab_test_group', values='visit_date').reset_index()
